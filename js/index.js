class juego {
  constructor() {
    ...
    this.containerPergerakan = document.getElementById('pergerakan');
  ...
  async iniciarJuego() {
    ...
    for (let i = 0; i < this.tarjetas.length; i++) {
      ...
      this.tarjetas[i].innerHTML =
        '<div class="front vueltaFront" data-position="' + i + '"></div><div class="back vueltaBack" data-position="' + i + '" style="background-image: url(' + this.PersonajeTemporal.image + ');">' + '' + '</div>';
    ...
    if (this.time === true) {
      switch (this.NTarjetasSeleccionadas) {
        case 0:
          ...
          this.pergerakan++;
          this.containerPergerakan.innerText = `Pergerakan: ${this.pergerakan}`;
          break;
        case 1:
          this.pergerakan++;
          this.containerPergerakan.innerText = `Pergerakan: ${this.pergerakan}`;
  ...
  victoria() {
    this.PausarTiempo();
    swal('Kamu Berhasik!', `Pergerakan Yg Kamu Lakukan: ${this.pergerakan} \n\n Lama waktu Yg kamu selesaikan: ${this.tiempo}`, 'success').then(() => {
      console.log('hola');
    });
  }
